...
def extract(url: str, output: str) -> dict:
    try:
        ...
        return {"ok": False, "url": url, "output": output, "output_bytes": 0, "error": str(exc)}
    ...
    text = _content_text(page)
    if not text.strip():
        return {"ok": False, "url": final_url, "status": status_i, "output": output, "output_bytes": 0, "error": "extract output missing or empty"}
    if len(text) > MAX_EXTRACT_CHARS:
        return {"ok": False, "url": final_url, "status": status_i, "output": output, "output_bytes": 0, "error": f"extract content exceeds {MAX_EXTRACT_CHARS} character safety limit"}
    out = Path(output)
