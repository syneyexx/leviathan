...
def chat(
    ...
    *,
    ...
    key = api_key or os.environ.get("HADES_LM_STUDIO_API_KEY") or os.environ.get("OPENAI_API_KEY") or "lm-studio"
    if key:
    ...
    try:
        ...
        detail = _redact_detail(exc.read().decode("utf-8", errors="replace")[:2000])
        return {"ok": False, "error": _map_http_error(exc.code), "http_status": exc.code, "detail": detail, "url": url}
    ...
    reported_model = str(data.get("model") or model_id) if isinstance(data, dict) else model_id
    return {
        "ok": True,
