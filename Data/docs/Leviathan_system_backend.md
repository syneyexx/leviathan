...
**P0A (kernel honesty — CURRENT):** `SimFill` carries measured honesty fields (`realized_delta`, `remaining_qty`, `order_type`, `fill_price_source`, `observed_execution`, `decision_bar_index`, `intent_id`, `trade_id`) — unmeasured fields are omitted from `public_dict`. `ClosedTrade` / `PositionEpisode` (`position_episodes.py`) is the foundation for closed-trade win rate; fill-level win rate is separately labeled and must not be conflated. `resolve_periods_per_year` annualizes Sharpe/Sortino via instrument calendar → observed frequency → asset-family default → timeframe fallback (crypto 1h ≠ equity 1h); unresolved annualization yields UNMEASURED. `evaluate_acceptance` reads `compute_metrics` keys (`trade_count`, `total_return`/`total_return_pct`, `max_drawdown`/`max_drawdown_pct`). Migration 45 persists honesty fill columns and `market_sim_closed_trades`. Single-engine still uses legacy FillModel bridge (P0B migrates fully to `NextBarFillModel` + WalletLedger). G08–G11 remain NOT_STARTED until full P0.
