...
**P0B (one execution path — CURRENT):** `SimulationEngine.step_once` uses only `WalletLedger` + `RiskGuard` + `NextBarFillModel` (no LegacyFill in the hot path). `fill_model.FillModel` is a thin Portfolio↔WalletLedger compatibility shim. `OrderIntent` carries `order_type` (MARKET/LIMIT/STOP/STOP_LIMIT), `limit_price`, `stop_price`, `time_in_force` (BAR/IOC/FOK/GTC/DAY). MARKET default TIF=BAR cancels partial remainder; GTC/DAY leave WORKING remainder. Limit/Stop use next-bar OHLC trigger rules with explicit gap handling; STOP_LIMIT arms then acts as LIMIT (no guaranteed fill). `IntrabarPathPolicy` (default CONSERVATIVE) resolves OHLCV stop+target ambiguity without silent favorable assumptions. Shared single-agent wallet is allowed. G08 IN_PROGRESS (golden fills); G09–G11 still NOT_STARTED (P0C/P0D).
