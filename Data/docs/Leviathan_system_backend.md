...
**P3A (orchestra cadence + async DecisionRecord — CURRENT):** `decision_cadence.py` owns explicit cadence gates (`every_n_bars` / `daily_close` / `hourly` / `event_driven` / `off`) and `AsyncDecisionQueue` (causal as_of eligibility, no rewind). `create_run` no longer injects a silent multi-agent roster (D31); cadence is recorded on run metadata. Engines consult `should_decide_on_bar`. Trading Orchestra registers `AgentDefinitionKind.TRADING` executors on Agent Fleet; append-only `DecisionRecord` chain (proposal→critique→risk→intent). `AgentKind.TRADING` labeled. G24 PASS.
