...
**P0C (risk/sizing/instruments — CURRENT):** `RiskGuard.on_bar_timestamp` resets `orders_today` on UTC calendar-day change (wired in SimulationEngine + MultiAgentEngine). Explicit `SizingModel` (`risk_pct` default, `fixed_qty`, `fixed_notional`, `all_in_cap`) on `SimRun.sizingModel` — undocumented `risk_qty * 5` removed. Instrument lot/tick/min_notional via `validate_intent_rules`. Opening short requires `supports_short` **and** `ShortMarginPolicy` else `MARGIN_POLICY_REQUIRED` / `INSTRUMENT_RULE`. G10 PASS.

**P0B (one execution path):** `SimulationEngine.step_once` uses only `WalletLedger` + `RiskGuard` + `NextBarFillModel`. `fill_model.FillModel` is a thin compatibility shim. TIF/Limit/Stop/IntrabarPathPolicy as documented. G08 IN_PROGRESS.
