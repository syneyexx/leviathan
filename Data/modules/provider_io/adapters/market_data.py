...
class MarketDataAdapter:
    ...
    def execute(
        ...
        *,
        ...
        del clients, credential, policy  # market providers use urllib today
        ...
        registry = default_registry(markets_root)
        try:
            result = registry.import_to_csv(
                provider_id, symbol, timeframe, markets_root, limit=limit
            )
        except MarketSimError as exc:
            raise ProviderError(
                ProviderErrorCode.PROVIDER_UNAVAILABLE
                if getattr(exc, "code", "") not in {"PROVIDER_UNKNOWN", "PROVIDER_EMPTY"}
                else ProviderErrorCode.PROVIDER_INVALID_REQUEST,
                str(exc),
                provider=provider_id,
                retryable=False,
                details={"market_code": getattr(exc, "code", None)},
            ) from exc
        return ProviderExecutionResult(
            ...
            provider=provider_id,
            finish_reason="stop",
            metadata={"capability": "market.fetch", "symbol": symbol, "timeframe": timeframe},
        )
