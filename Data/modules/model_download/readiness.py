"""Helpers to decide when Control Plane may enqueue model_download work."""
...
def model_download_workers_ready(database_path: Any | None = None) -> bool:
    """True when at least one model_download worker is READY or BUSY."""
    try:
        from Data.modules.workers.settings import load_worker_settings

        if load_worker_settings().desired_count("model_download") <= 0:
            return False
    ...
    try:
        ...
        registry.initialize()
        workers = registry.list(pool_id="model_download")
        return any(
