...
class OpenAICompatibleLLM:
    ...
    /v1/chat/completions. The client is intentionally provider-light so the model
    layer can be replaced later without changing chat or knowledge persistence.
    """
    ...
    async def chat(
        ...
        if knowledge:
            ...
            for item in knowledge:
                ...
                chunks.append(
                    f"SOURCE: {item['title']} ({item.get('source', 'unknown')})\n{excerpt}"
                )
            knowledge_block = (
