"""Phase T0 characterization tests for Market Sim defects D1–D21.

Each test is tagged with its defect ID. Tests assert the *desired* post-fix
contract and are marked ``expectedFailure`` until the fixing phase turns them
green. Companion tests assert *current* broken behaviour so Gap Report evidence
stays reproducible without waiting for a fix.
...
class D16GatewayCharacterization(unittest.TestCase):
    ...
    @unittest.expectedFailure  # D16 — fixed in Phase T5
    def test_d16_desired_market_capabilities_in_catalog(self) -> None:
        from Data.modules.execution.builtins import build_default_catalog

        catalog = build_default_catalog()
        ids = {c.id for c in catalog.list()}
        self.assertTrue(any(i.startswith("market.") or i.startswith("sim.") for i in ids))
...
class D19SecretsCharacterization(unittest.TestCase):
    ...
    @unittest.expectedFailure  # D19 — fixed in Phase T5
